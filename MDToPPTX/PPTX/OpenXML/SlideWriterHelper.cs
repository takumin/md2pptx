...
namespace MDToPPTX.PPTX.OpenXML
{
	class SlideWriterHelper
	{
		...
		public static int CreateHyperLinkMap(
			PPTXSlide SlideContent,
			SlidePart slidePart1,
			Dictionary<string, string> HyperLinkIDMap
		)
		{
			int lastIndex = 2;

			var textRuns = SlideContent
				.TextAreas.SelectMany(_textArea => _textArea.Texts.SelectMany(_text => _text.Texts))
			...
			foreach (var linkItem in textRuns)
			{
				...
				slidePart1.AddHyperlinkRelationship(
					new System.Uri(linkItem.Link.LinkURL, System.UriKind.Absolute),
					true,
					linkId
				);
		...
		public static A.ParagraphProperties CrateParagraphProperties(PPTXText Content)
		{
			...
			switch (Content.Bullet)
			{
				...
				case PPTXBullet.CircleNumber:
					...
					paragraphPorp.Append(
						new A.AutoNumberedBullet() { Type = A.TextAutoNumberSchemeValues.CircleNumberDoubleBytePlain }
					);
					break;
			}
		...
		public static A.RunProperties CreateRunProperties(PPTXTextRun Text, Dictionary<string, string> HyperLinkIDMap)
		{
			A.RunProperties runProperties3 = new A.RunProperties()
			{
				...
				FontSize = (int)(Text.Font.FontSize * 100),
				Dirty = false,
			};
		...
		public static A.RgbColorModelHex CreateRGBColorModeHex(PPTXColor Color) =>
			new A.RgbColorModelHex() { Val = $"{Color.Color.R:X02}{Color.Color.G:X02}{Color.Color.B:X02}" };
	}
}
