	...
	public class SlideManager
	{
		...
		public void WriteImage(PPTXImage Image)
		{
			if (System.IO.File.Exists(Image.ImageFilePath))
			{
				...
				using (SkiaSharp.SKBitmap bitmap = SKBitmap.Decode(Image.ImageFilePath))
				{
		...
		public PPTXTransform NewTransform()
		{
			...
			return new PPTXTransform(
				Settings.Margin.Left + margin.Left,
		...
		public void EndSheet(PPTXSlide CurrentSheet = null)
		{
			...
			if (Settings.StackLayoutRule == EPPTXSideStackLayoutRule.Center)
			{
				...
				var contentCentetY =
					((LastAddedItemTransform.PositionY + LastAddedItemTransform.SizeY) - Settings.Margin.Top) / 2.0f
					+ Settings.Margin.Top;
