...
namespace MDToPPTX
{
	public class MD2PPTX
	{
		...
		public static void RunFromMDText(string MarkdownText, string ExportPath, PPTXSetting options = null) =>
			ToPPTX(MarkdownText, ExportPath, options);

		protected static void ToPPTX(
			string markdown,
			string pptxFilePath,
			PPTXSetting options = null,
			MarkdownPipeline pipeline = null
		)
		{
			options = options ?? new PPTXSetting() { SlideSize = EPPTXSlideSizeValues.Screen4x3 };
		...
		/// <param name="ExportPath">pptx file path</param>
		/// <param name="options">Option setting</param>
		public static PPTXDocument MakePPtxDocumentFromMDText(string MarkdownText, PPTXSetting options = null) =>
			ToPPTxDocument(MarkdownText, options);

		protected static PPTXDocument ToPPTxDocument(
			string markdown,
			PPTXSetting options = null,
			MarkdownPipeline pipeline = null
		)
		{
			...
			pipeline = pipeline ?? new MarkdownPipelineBuilder().UsePipeTables().UseEmphasisExtras().Build();
